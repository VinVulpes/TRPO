...
int main()
{
    ...
    if (string(getenv("REQUEST_METHOD"))==string("GET")){
        if(http.httpGet("getcookie")!=""){
            ...
            cout<<http.httpGet("getcookie") <<": " <<http.getCookie(http.httpGet("getcookie"))<<"<br>";
        }
        if(http.httpGet("setcookie")!=""){

            cout <<"<br>\nRequested Set Cookie: "<<"<br>" <<endl;
            cout<<http.httpGet("setcookie") <<": " <<http.setCookie(http.httpGet("setcookie"),http.httpGet("value"))<<"<br>";
        }
    }
